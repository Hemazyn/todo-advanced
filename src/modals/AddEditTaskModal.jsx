...
function AddEditTaskModal({ type, device, setOpenAddEditTask, taskIndex, prevColIndex = 0, }) {
     ...
     return (
          <div className={
               device === "mobile"
                    ? "  py-6 px-6 pb-40  absolute overflow-y-scroll  left-0 flex  right-0 bottom-[-100vh] top-0  bg-[#00000080] dropdown "
                    : "  py-6 px-6 pb-40  absolute overflow-y-scroll  left-0 flex  right-0 bottom-0 top-0 bg-[#00000080] dropdown "
          }
               onClick={(e) => {
               ...
               <div className=" scrollbar-hide overflow-y-scroll max-h-[95vh]  my-auto  bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e1a] max-w-md mx-auto  w-full px-8  py-8 rounded-xl">
                    ...
                    <div className="mt-8 flex flex-col space-y-1">
                         ...
                         <input value={title} onChange={(e) => setTitle(e.target.value)}
                              id="task-name-input" type="text" placeholder=" e.g Take coffee break"
                    ...
                    <div className="mt-8 flex flex-col space-y-1">
                         ...
                         <textarea value={description}
                              onChange={(e) => setDescription(e.target.value)} id="task-description-input"
                    ...
                    <div className="mt-8 flex flex-col space-y-3">
                         ...
                         {subtasks.map((subtask, index) => (
                              <div key={index} className=" flex items-center w-full ">
                                   <input onChange={(e) => { onChangeSubtasks(subtask.id, e.target.value) }}
                                        type="text" value={subtask.title} placeholder=" e.g Take coffee break"
                                   ...
                                   <img src={crossIcon} onClick={() => { onDelete(subtask.id) }} className="m-4 cursor-pointer" />
                              </div>
                         ))}
                    ...
                    {/* current Status  */}
                    <div className="mt-8 flex flex-col space-y-3 relative">
                         ...
                         <select value={status} onChange={(e) => onChangeStatus(e)}
                              className="relative select-status flex flex-grow px-4 py-2 rounded-md text-sm bg-transparent focus:border-0 border-[1px] border-gray-300 focus:outline-[#635fc7] outline-none">
                              {columns.map((column, index) => (
                                   <option value={column.name} key={index} className="bg-red-800 text-white">{column.name}</option>
                              ))}
                         </select>
                         <button
                              onClick={() => {
